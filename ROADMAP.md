...
- ✅ **`/api` → its own Cloudflare Workers Build (DONE, 2026-06-20):** the API worker now auto-deploys on every push to `main` that touches `/api` (root dir `api`, `npm ci` + `npx wrangler deploy`, watch path `api/*`, production-only — non-production branch builds disabled). Manual `cd api && npm run deploy` still works for ad-hoc deploys.
...
## `www → apex` canonical redirect — DONE (2026-06-20)

A Cloudflare Redirect Rule ("Redirect from WWW to root") now 301-redirects `www.awonderfullife.ca/*` → `https://awonderfullife.ca/${1}`, preserving path + query. Verified live: `www/`, `www/archive`, and `www/posts/…?ref=…` all 301 to the apex with path + query intact, and the apex itself stays 200 (no loop). Search engines now see one canonical host.
