...
## v3.5.4 — New post: "The Wait Was Worth It: iOS 27's Siri AI" (2026-07-16 UTC)

A new technology essay, and the third in an accidental trilogy. It revisits the October 2024 Apple Intelligence review and the March 2025 Siri preview, then measures both against the iOS 27 public beta (released July 13, 2026) and its rebuilt "Siri AI." The piece traces the full two-year arc: the personal Siri promised and marketed on the iPhone 16, the public delay that followed, the smaller wins Apple quietly shipped in iOS 26 while the headline feature waited, and the on-screen-aware, context-reading, action-taking assistant finally arriving now, and argues that the painful wait produced a better, better-architected result. It is first-person and reflective, and, because iOS 27 postdates the model's training, every factual claim is grounded in current reporting (Apple Newsroom, TechCrunch, CNBC, MacRumors, 9to5Mac, AppleInsider, Macworld) rather than memory, with internal links to both prior posts and the "Built With AI" essay. It is also the first post in the series to carry in-body figures: a photoreal hero plus three inline illustrations (`gpt-image-1`, matching the refreshed series), which prompted a small `.post-content img` style (rounded corners, block layout). Everything downstream, the homepage feature slot, the feed, the sitemap, and the SEO head and JSON-LD (`BlogPosting` + `BreadcrumbList`), is generated from the one Markdown file as usual.

## v3.5.3 — Humanized excerpts + balanced title optimization (2026-07-16 UTC)

The reader-facing companion to the v3.5.1 description sweep. Every post's homepage excerpt — the teaser you actually see on the front page and the category cards — was rewritten into a first-person voice. Most had been either flatly robotic ("The author reflects on…") or written in a polished-but-detached editorial-blurb voice; now all 17 sound like Vijay talking. In the same pass, every title got a balanced SEO tune: the evocative hook stays, and the subtitle is reworked to carry the phrase a reader would actually search — "Whispers of Wanderlust: An Eco-Friendly Journey Across Canada" becomes "…: An Electric-Car Road Trip Across Canada," and so on. Changing a title updates the heading, the browser title, the cards, the navigation, the social tags, the structured data, and the feed all at once, but it deliberately leaves the URL (the slug) and the frozen RSS GUID alone, so no links break and no subscriber sees a phantom "new" post. Only the title and excerpt changed; the essays, dates, and meta descriptions are as they were.

## v3.5.2 — Fix: cache-bust the About portrait (2026-07-16 UTC)

The new portrait shipped in v3.5.1 reused the filename `vijay.jpg`, but the site caches everything under `/assets/images/` as immutable for a year — a rule that suits the uniquely-named hero images but means a file swapped in place keeps serving its old bytes to the Cloudflare edge and to returning browsers long after a redeploy. On production the plain image URL was still returning the old 595×592 portrait while the new 800×800 file was reachable only with a cache-busting query string. The fix renames the served portrait to `vijay-v2.jpg` and repoints all four references (the About image, the per-post author avatar, the About social image, and the `Person` entity image) at the fresh URL. The underlying rule — rename stable-name images instead of overwriting them — is now written down in CLAUDE.md so the next portrait or default-social-image change does not hit the same trap.

## v3.5.1 — First-person meta descriptions + new About portrait (2026-07-16 UTC)

Every post's meta description was rewritten. The old ones were machine-style third-person summaries ("The author reflects…"), several cut off mid-word, and two were just the title pasted twice — the kind of thing that reads as broken in a search result and earns no clicks. Each of the 17 is now a complete first-person sentence (134–148 characters) that carries a phrase a real reader might search, while the titles, the essays themselves, and the visible homepage card excerpts stay exactly as they were. The About page also gained a new editorial-illustration portrait, optimized to 800×800 and, at 73 KB, smaller than the photo it replaces; it doubles as the site's `Person` entity image and the About page's social-share card, with its dimensions picked up automatically by the build's `imageDims` filter. No template or structural change — a content pass on top of the v3.5.0 SEO plumbing.

## v3.5.0 — SEO structured-data + social metadata (2026-07-16 UTC)

The generated `<head>` gained a set of discoverability signals, layered on top of the existing SEO block without changing any of it. The `Person` in the JSON-LD `@graph` now lists Vijay's profiles (`sameAs`: LinkedIn, X, GitHub, Track Your App, Instagram), so a search engine can consolidate the scattered mentions of "Vijay Panangipally" into a single entity. Every post now also carries a `BreadcrumbList` (Home › Category › Post), making it eligible for breadcrumb rich results. Open Graph and Twitter cards were rounded out: images gained `og:image:alt` and real `og:image:width`/`height` — measured from each JPEG at build time by a small dependency-free filter, because the OG images are not a uniform size and a single hardcoded pair would be wrong for most pages — and the cards gained `twitter:site`/`twitter:creator` attribution and image alt text. Every page gained a `<meta name="author">`. A new optional `updated` front-matter field lets a meaningfully-edited post advertise a real freshness date (`dateModified` / `article:modified_time`); when it is absent both fall back to the publish date, so nothing changed for the existing posts. This shipped alongside a separate decision to open `robots.txt` fully to every crawler, including AI engines, so the essays can surface in AI answer engines as well as classic search.

## v3.4.0 — Appreciate button on posts (2026-07-11 UTC)

A small, zero-PII "appreciate this" button now sits at the end of every post, between the essay and the related-reading strip. It reads its count from the owned reactions API and increments on a click, remembering (via `localStorage`, no cookies) that a visitor already appreciated a post so they are not prompted to do it twice. The client (`assets/js/reactions.js`) follows the same discipline as the comments widget: a tiny dependency-free script, all text rendered with `textContent` (no injection surface), the count coerced through a numeric guard, and graceful silence if the API is unreachable — the button never blocks or breaks the page. It sits outside the search-indexed article, loads only on posts, and styles itself from the existing design tokens, so it reads correctly in light and dark. The counter is only ever set from a live server response; `localStorage` is a convenience, and the server's salted daily per-IP throttle remains the real guard.

## v3.3.0 — Related posts (2026-07-11 UTC)

Each post now ends with a "More reading" strip of three related posts, computed at build time (no runtime cost). Selection prefers the same category, newest first, excludes the post itself, and backfills from the most recent posts in other categories when a category is thin — so every post shows a full set of three. The cards reuse the existing homepage card component, so they match the rest of the site and inherit dark mode automatically. The strip sits outside the search-indexed article, so related links never leak into a post's own search excerpt.

## v3.2.0 — Static search (Pagefind) on the archive (2026-07-11 UTC)

The archive page gained an instant search box, above the faceted browse strip. It is powered by [Pagefind](https://pagefind.app), which indexes the built site at build time into a self-contained bundle served from the site itself — no server, no external service, no index to maintain, nothing to pay for. The index is scoped to post bodies only, so results are essays (not navigation chrome or category-listing pages), and prev/next navigation text is excluded from excerpts. The search UI loads only on the archive page (zero weight on every other page), degrades gracefully without JavaScript (the full archive list is right below it), and is themed through Pagefind's CSS variables to match the site in both light and dark.

## v3.1.0 — Dark mode (2026-07-11 UTC)

The site now has a dark theme: it follows the reader's operating-system preference by default and can be flipped manually with a new toggle in the header, with the choice remembered across visits. An inline script in the document head applies the saved preference before the first paint, so there is no flash of the wrong theme on load. Under the hood this was a two-step change: first the last ~20 hardcoded colours in the stylesheet were pulled into semantic design tokens (a pure refactor, no visual change), then a dark palette was defined against those tokens — a deep slate background, soft off-white text, and a lightened accent tuned to stay legible (WCAG AA) on dark. The Liquid-Glass surfaces, the homepage ambient backdrop, the newsletter and comment forms, and the search widget were all re-tuned so nothing glares or washes out at night. No new dependencies, no external requests.

## v3.0.0 — Re-platform to Eleventy (2026-07-11 UTC)

The site moved from hand-authored HTML to a build: **Eleventy** (`@11ty/eleventy` v3) now generates every page from Markdown posts and Nunjucks templates. Posts live at `src/content/posts/<slug>.md` — one file each, front matter + body — and `npm run build` renders the homepage, about, archive, all 5 category pages, all 17 posts, `feed.xml`, and `sitemap.xml` into `_site/`, which is what Cloudflare now serves (`wrangler.jsonc`'s `assets.directory` moved from `.` to `./_site`). The SEO head block (canonical, OG/Twitter, JSON-LD `@graph`) is likewise generated, by `src/_includes/head-seo.njk` and an `eleventy/jsonld.mjs` graph builder that mirrors the retired injector. Zero visual or URL change for readers — every route, every byte of rendered content, is the same as the site this replaces.

**Byte-fidelity, proven, not assumed.** A dedicated parity gate (`scripts/parity-check.mjs`, since deleted along with the rest of the one-time migration tooling) diffed every generated page against its legacy counterpart — head tags, body DOM, feed items, sitemap entries — down to attribute order and whitespace. It ran 27/27 PASS before a single legacy file was deleted. Building that gate surfaced five latent bugs already live on the legacy site, each adjudicated and corrected in the generated output rather than silently reproduced:

- **Stale reading time.** The homepage featured card for "Built With AI, Here to Stay" still read "7 MIN READ" after the post was rewritten to 9 minutes in v2.17.5 (the technology category card had the same fossil). Generated output computes it fresh.
- **Double-HTML-escaped feed titles.** Three `feed.xml` `<title>` values decoded to literal `&#x27;`/`&quot;` instead of the real character — a genuine RSS-reader-facing bug present since the feed was first added in v2.15.0. Generated feed emits the correct character.
- **Missing period-facet chip.** `categories/technology.html`'s faceted-browsing strip never got a "2026" year chip when the first 2026 post landed in that category (v2.17.3) — every other year/category combination already had one. Generated output includes it.
- **A straight-apostrophe typo, four surfaces.** The homepage card, the technology category card, and both neighboring posts' prev/next nav all spelled the Apple-Intelligence post's title with a straight `'` where the post's own `<h1>` (and every other reference to it) uses a curly `'`. Generated output is consistent everywhere.
- **Inconsistent same-date post order.** The two November 18, 2024 posts were ordered one way in `archive.html`, `index.html`, and the hand-authored prev/next chain (six surfaces, in agreement), and the other way in `feed.xml` (added later, v2.15.0, and never reconciled). The generator matches the six-surface dominant convention.

**Retired:** the "four-plus-places" post-editing workflow (a new post used to mean hand-edits across the post file, the homepage, the archive, and its category page, plus manual prev/next nav updates on its neighbors) and `docs/superpowers/tools/seo-inject.py` (the one-time Python script that regex-parsed hand-authored HTML to inject the SEO block) are both gone — a post is now one Markdown file, and everything downstream of it is generated. `scripts/migrate-posts.mjs` and `scripts/parity-check.mjs`, the one-time tooling that got the corpus from HTML to Markdown and proved it faithful, are deleted too (their job is done; git history keeps them). `cheerio` and `turndown` (only used by those scripts) are dropped from `devDependencies`; an `engines.node >=18` floor is pinned for a deterministic Workers Build. `.assetsignore` is deleted — the asset directory is now `_site/`, which never contains anything but build output, so there's nothing left to exclude.

Also fixed in passing: the Eleventy passthrough copy no longer drags `assets/staging-images/` (gitignored pre-optimization source art) into `_site/`.

(The dormant zero-PII "appreciate" reactions API in `/api`, shipped separately and not yet wired to any client, is unaffected by this re-platform.)

## v2.17.5 — "Built With AI, Here to Stay": full editorial rewrite from the owner's edited draft (2026-07-10 UTC)

Replaced the post body with the owner's own edited draft (supplied as a .docx), matching both its content and its formatting. Structurally this is a different piece from the v2.17.3 original: seven `<h2>` section breaks (`It Started With a Spreadsheet`, `What "AI-Assisted" Really Looked Like`, `The Most Important Decision Was Not Technical`, `Then We Made a Serious Mistake`, `Trust Is Built After the Launch`, `Judge the Software, Not the Toolchain`, `What Actually Earned Trust`), a new italicized dek under the headline (`.post-dek`, new CSS rule added to `style.css`, styled after the existing `.post-hero-caption` treatment), two bolded pull-lines (`How much is actually moving?` and `Built with AI.`), and a much shorter-paragraph, staccato rhythm throughout. Content additions include an explicit new "Judge the Software, Not the Toolchain" section conceding that careless AI-generated software genuinely exists, and a franker line on the data-loss incident ("the fact that recovery systems worked does not excuse the action that made them necessary"). Byline updated to 9 MIN READ (1,769 words). No changes to hero image, nav, or metadata.

Swapped the post's hero for a photoreal image the owner generated and dropped into `assets/staging-images/` (gitignored/asset-ignored source folder, matching how the original 16-post photoreal series was produced), replacing the `gen-hero.sh` flat-editorial-style placeholder from v2.17.3. Same concept (a cluttered desk of spreadsheets dissolving into a glowing pathway toward a maple leaf), rendered in the site's actual established hero style — the other 15 posts are all photoreal, 3:2 (1536×1024); the v2.17.3 placeholder was the one 16:9 flat-editorial outlier. Center-cropped from the 16:9 source to 3:2 and resized to match, hero + thumbnail regenerated, `alt` text rewritten to describe the new image. No HTML structure changes beyond the `alt` attribute (filename unchanged).
...
**Fix:** `docs/superpowers/tools/seo-inject.py`'s `page_url()` was appending the raw `.html` extension to every page's canonical/OG/JSON-LD URL, contradicting the site's extension-less canonical convention (`/archive`, not `/archive.html`) that every existing page already followed (evidently patched by hand after generation, previously). Now strips `.html` before building the URL, so future posts generate correct URLs without a manual fix-up. No existing files touched (the script already skips pages with a JSON-LD block).
...
## v2.16.7 — About portrait: rounded corners (2026-06-18 13:45 UTC)

Bumped the About-page portrait's `border-radius` 4px → 14px to match the rounded corners now used site-wide (heroes, cards, comment cards).
