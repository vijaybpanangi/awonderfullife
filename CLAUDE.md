...
src/_data/site.json             Site-wide data: name, tagline, url, author, categories, sameAs, twitterHandle
...
- `updated` — **optional** `YYYY-MM-DD`. When present it drives the post's JSON-LD `dateModified` and the `article:modified_time` OG tag; when absent both fall back to `date`. Set it after a meaningful edit to an already-published post so search engines see a real freshness signal. It does not change the visible byline.
...
Every page carries, before `</head>`: an absolute self-canonical, `<meta name="author">`, Open Graph + Twitter Card tags (`og:type` is `article` on posts, `website` elsewhere), and a schema.org JSON-LD `@graph`. The graph shares a `WebSite` + `Person` (Vijay Panangipally) + `Blog`, with a per-page node: posts are `BlogPosting` (headline/date/image/section parsed from front matter), other pages are `CollectionPage` / `AboutPage`. Posts additionally emit a `BreadcrumbList` node (Home › Category › Post). The `Person` carries `sameAs` (Vijay's profile links) and the cards carry `twitter:site`/`twitter:creator`, both sourced from `src/_data/site.json` (`sameAs` array + `twitterHandle`). This block is generated by `src/_includes/head-seo.njk` (canonical + OG + Twitter) and the `jsonld` include, backed by `eleventy/jsonld.mjs` (the `buildGraph()` function, registered as the Nunjucks global `jsonLdGraph`). **The old `docs/superpowers/tools/seo-inject.py` script — a one-time Python injector that regex-parsed hand-authored HTML — is retired**; there is nothing to re-run after adding a post, the build produces the same block automatically. `feed.xml`, `sitemap.xml`, and `robots.txt` are likewise generated (feed/sitemap from the posts collection; `robots.txt` is still a static passthrough file at the repo root — currently fully open to all crawlers, including AI, per the 2026-07-16 decision). `og:image`: posts use their hero, About uses `vijay-v2.jpg`, other pages use `assets/images/og-default.jpg` — each carries `og:image:alt` and real `og:image:width`/`height` measured from the JPEG at build time by the dependency-free `imageDims` filter (`eleventy/filters.mjs`), since the images are not a uniform size.

**Cache-busting gotcha for stable-name images.** `_headers` caches `/assets/images/*` as `Cache-Control: public, max-age=31536000, immutable`. That is correct for hero images (each new post has a unique filename), but it means any image that is *replaced under the same filename* will be served stale for up to a year by both the Cloudflare edge and returning visitors' browsers — a plain redeploy does not fix it. When you change a stable-name image (the author portrait, `og-default.jpg`), **give it a new filename** (e.g. `vijay-v2.jpg` → `vijay-v3.jpg`) and update its references, rather than overwriting in place. The portrait was renamed to `vijay-v2.jpg` in `v3.5.2` for exactly this reason (referenced from `about.njk`, `author-card.njk`, `head-seo.njk`, and `eleventy/jsonld.mjs`).
